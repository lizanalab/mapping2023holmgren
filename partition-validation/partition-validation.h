...
public:
    ...
    Partitions(const string &inFileName,
        ...
        if (!ifs) {
            cout << "Failed to open \"" << inFileName << "\" exiting...\n";
            exit(-1);
        }
...
void Partitions::clusterPartitions(int fold) {
    ...
    cout << "Clustering " << nTrainingPartitions << " partitions (modified algorithm):\n";
    ...
    vector<int> unAssignedPartitionIndices;
    ...
    for (int i = 1; i < nTrainingPartitions; i++) {
        ...
        for (int j = 0; j < nClusters; j++) {
            if (wpJaccardDist(partitionPtrs[i], clusters[j][0]) < distThreshold) {
                fitsInCluster = true;
                unAssignedPartitionIndices.push_back(i);
    ...
    for (auto i : unAssignedPartitionIndices) {
        auto minDist = 1.0;
        auto bestClusterIdx = -1;

        for (int j = 0; j < nClusters; j++) {
            auto dist = wpJaccardDist(partitionPtrs[i], clusters[j][0]);
            if (dist < distThreshold && dist < minDist) {
                minDist = dist;
                bestClusterIdx = j;
            }
        }
        if (minDist < distThreshold && bestClusterIdx != -1) {
            clusters[bestClusterIdx].push_back(partitionPtrs[i]);
        } else {
...
#pragma omp atomic
    ...
    if (validationSamples == 0) {
        ...
        cout << "Writing validation results to " << validationOutFileName << '\n';

        my_ofstream ofs;
        ofs.open(validationOutFileName.c_str());

        for (int i = 0; i < nValidationPartitions; i++) {
            ofs << validationPartitionPtrs[i]->partitionId + 1 << " " << validatedPartitions[i] << '\n';
        }

        ofs.close();
    } else {
        validationOfs << nValidated << " " << nClusters << '\n';
    }
}
...
void Partitions::readPartitionsFile() {
    ...
    if (nTrainingPartitions == 0) {
        if (nPartitions - nValidationPartitions > 0) {
            ...
            cout << "-- Not enough partitions for validation. Will not validate.\n";
        }
    } else if (nPartitions - nValidationPartitions - nTrainingPartitions < 0) {
        cout << "-- Not enough partitions for training and validation.\n";
    }
...
void Partitions::printClusters() {
    ...
    int i = 1;
    ofs << "# Clustered " << nTrainingPartitions << " partitions into " << clusters.size() << " clusters.\n";
    ofs << "ClusterId PartitionId\n";
    for (auto &cluster: clusters) {
        for (auto &partition: cluster) {
            ofs << i << " " << partition->partitionId + 1 << '\n';
        }
        i++;
    }
    ...
    for (auto cluster1 = begin(clusters); cluster1 != end(clusters); cluster1++) {
        ...
        for (auto cluster2 = next(cluster1); cluster2 != end(clusters); cluster2++) {
            ...
            ofs << i << " " << j << " " << wpJaccardDist(partition1, partition2) << '\n';
            j++;
        }
        i++;
    }
    ...
    i = 0;
    ofs << "# Clustered " << nTrainingPartitions << " partitions into " << clusters.size() << " clusters.\n";
    ofs << "partition1 partition2 distance\n";

    for (auto partition1 = begin(partitions); partition1 != end(partitions); partition1++) {
        auto p1 = &(*partition1);
        auto j = i + 1;
        for (auto partition2 = next(partition1); partition2 != end(partitions); partition2++) {
            auto p2 = &(*partition2);
            ofs << i << " " << j << " " << wpJaccardDist(p1, p2) << '\n';
            j++;
        }
        i++;
    }
